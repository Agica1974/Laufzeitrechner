...
// function calculateMaterialkosten() {
//   // Eingabewerte abrufen und auf Leerzeichen prüfen
//   let materialpreis = parseFloat(document.getElementById("materialpreis").value.replace(",", "."));
//   let verbrauchProMeter = parseFloat(document.getElementById("verbrauchProMeter").value.replace(",", ".")) / 1000;
//   let gesamtlaengeInMetern = calculateGesamtlaenge();

//   // Prüfung, ob Felder gefüllt sind und positive Werte haben
//   if (isNaN(materialpreis) || isNaN(verbrauchProMeter) || materialpreis <= 0 || verbrauchProMeter <= 0) {
//     document.getElementById("materialkosten").value = 'Materialkosten wurden nicht berechnet';
//     materialkostenField.classList.add("no-calc-background");
//   } else {
//     // Berechnung der Materialkosten
//     let materialkostenResult = materialpreis * verbrauchProMeter * gesamtlaengeInMetern;
//     document.getElementById("materialkosten").value = roundToTwo(materialkostenResult) + ' € Materialkosten';
//     materialkostenField.classList.remove("no-calc-background");
//   }
// }
...
function calculateMaterialkosten() {
  ...
  let verbrauchProMeter = parseFloat(document.getElementById("verbrauchProMeter").value.replace(",", ".")) / 1000;
  let gesamtlaengeInMetern = calculateGesamtlaenge();
  let materialkostenField = document.getElementById("materialkosten");

  // Prüfen, ob Eingaben gültig sind
  if (!isNaN(materialpreis) && !isNaN(verbrauchProMeter) && materialpreis > 0 && verbrauchProMeter > 0) {
    let materialkostenResult = materialpreis * verbrauchProMeter * gesamtlaengeInMetern;
    materialkostenField.value = roundToTwo(materialkostenResult) + ' € Materialkosten';

    // Hintergrundfarbe zurücksetzen
    materialkostenField.classList.remove("no-calc-background");
  } else {
    // Keine gültige Berechnung möglich
    materialkostenField.value = 'Materialkosten wurden nicht berechnet';

    // Hintergrundfarbe nur setzen, wenn keine Berechnung möglich ist
    materialkostenField.classList.add("no-calc-background");
  }
}
...
function resetData() {
  ...
  materialpreis.value = "";
  verbrauchProMeter.value = "";
  materialkosten.value = "";
...
function formatDate(date, includeYear = false) {
  ...
  const year = date.getFullYear(); // Neuer Teil: Jahr abrufen
